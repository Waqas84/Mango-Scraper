...
$.getJSON("/saved", function(data) {
    ...
    if(data.length == 0){
      $(".articleList").append("<div class='noArticles'>Looks like we don't have any saved articles </div>")
    }
    ...
    for (var i = 0; i < data.length; i++) {
      if (data[i].saved == true){
        ...
        $(".action"+i).append("<a class='waves-effect waves-light btn red deleteArt' data-id=" + data[i]._id + ">Delete</a>")
    ...
    .done(function(data) {
      ...
      if (data[0].note.length>0){
        for(i = 0; i<data[0].note.length; i++){
          ...
          $("#lastNote"+i).append("<a class='waves-effect waves-light btn red noteTrash' data-id=" + data[0].note[i]._id + "></a>")
